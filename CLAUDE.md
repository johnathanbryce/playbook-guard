...
- `analyze(contractId)`: shared pipeline entry. Per rule: retrieve -> flag -> firewall -> escalate. Suppress escalation for fabricated flags (firewall gate). Compute coverage (rules w/ top-1 sim >= 0.70). Result cache keyed (contract_hash + playbook_version), 24h TTL. Feeds both GET /analysis and SSE.
