...
- web/src/App.tsx — DONE (analysis wired): after ingest, "Analyze against playbook" button opens EventSource(/stream?contractId=). `rule` events append streaming rule cards (verdict pill + grounding=firewall.status badge w/ reasoning tooltip + escalate→team pill + cited span + reasoning); `done` sets summary line (coverage/verified/needs-review/fabricated/n-a/cached) + escalation drafts w/ DISPLAY-ONLY Send/Deny buttons. Closes ES on done/error (no auto-reconnect). Typecheck clean
...
## BUILD ORDER (live)
~~seed -> chunk -> embed -> ingest -> wire upload -> retrieve -> flag -> firewall -> firewall tests -> escalate -> analyze(+result cache) -> /analysis -> SSE -> wire frontend EventSource~~ -> (remaining polish) frontend live-SSE / raw-JSON toggle

## NEXT (John drives — do not auto-start)
- Frontend live-SSE / raw-payload toggle (show streamed cards OR the raw GET /analysis JSON) — the last of John's 6 steps. Not started.
