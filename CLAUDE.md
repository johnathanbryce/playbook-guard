...
- api/src/routes/stream.ts — GET /stream?contractId= SSE: calls analyze(contractId, onRule) — one `rule` event per RuleResult as it resolves, final `done` w/ summary. Warm cache: analyze() replays each cached rule through onRule then done (streaming UX identical). (router mounted; body stubbed)
- NEW api/src/routes/analysis.ts — GET /analysis?contractId= -> calls analyze(contractId) w/ NO onRule, returns full AnalysisResult JSON. MUST be a real server-side analyze() call, NOT accumulated SSE (John's hard constraint). (first-class integration surface; cached/idempotent)
...
## NEXT (John drives — do not auto-start; working through his 6 steps chunk by chunk)
- Step 2: GET /analysis route — calls analyze(contractId) w/ no onRule, returns AnalysisResult JSON. Real server-side call, NOT accumulated SSE.
- Then: GET /stream SSE route (onRule -> event, then done); frontend live-SSE / raw-JSON toggle.
