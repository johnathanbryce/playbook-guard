...
- api/src/routes/stream.ts — GET /stream?contractId= SSE: calls analyze(contractId, onRule) — one `rule` event per RuleResult as it resolves, final `done` w/ summary. Warm cache: analyze() replays each cached rule through onRule then done (streaming UX identical). (handler stub; wired app.get in index.ts)  <-- NEXT
